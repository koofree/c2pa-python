...
class TestSignerr(unittest.TestCase):
    ...
    def test_data_hash(self):
        data_hash = DataHash("test", "sha256")
        hash = data_hash.hash_from_stream(open("tests/fixtures/A.jpg", "rb"))
        assert hash is not None
        hash_str = hash.hex()
        self.assertEqual(
            hash_str, "f999fd78bfe8a83c96e468a078830ba94485bc1bc6fd086fb94a43bd29dd0f23"
        )

        self.assertEqual(
            hash_str,
            hashlib.sha256(open("tests/fixtures/A.jpg", "rb").read()).hexdigest(),
        )
